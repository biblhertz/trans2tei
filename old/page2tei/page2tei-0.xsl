    ...
    <TEI>
      <teiHeader>
        ...
        <tagsDecl>
                ...
                <rendition scheme="css" xml:id="k">font-variant:small-caps; font-style:normal !important</rendition>
  ...
  <xsl:template match="p:TextRegion" mode="text">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@type = 'page-number'">
        <fw type="page" place="bottom" facs="#facs_{$numCurr}_{@id}"><xsl:apply-templates select="p:TextLine" /></fw>
      </xsl:when>
      <xsl:when test="@type = ('quote')">
        <quote facs="#facs_{$numCurr}_{@id}">
          <xsl:apply-templates select="p:TextLine" />
        </quote>
      </xsl:when>
      <xsl:when test="@type = ('commentary')">
        <p facs="#facs_{$numCurr}_{@id}" type="commentary" rendition="#indent">
          <xsl:apply-templates select="p:TextLine" />
        </p>
      </xsl:when>
      <xsl:when test="@type = ('first-paragraph')">
        <p facs="#facs_{$numCurr}_{@id}" type="first" rendition="#aq">
          <xsl:apply-templates select="p:TextLine" />
        </p>
      </xsl:when>
      <xsl:when test="@type = ('paragraph-center')">
        <p facs="#facs_{$numCurr}_{@id}" type="center" rendition="#c">
          <xsl:apply-templates select="p:TextLine" />
        </p>
      </xsl:when>
      <xsl:when test="@type = ('bibliography', 'archival-notice')">
        <p facs="#facs_{$numCurr}_{@id}" type="{@type}{$custom?structure?type}" rendition="#smaller">
          <xsl:apply-templates select="p:TextLine" />
        </p>
      </xsl:when>
      <xsl:when test="@type = ('other', 'paragraph')">
        <p facs="#facs_{$numCurr}_{@id}" rendition="#aq #indent">
          <xsl:apply-templates select="p:TextLine" />
        </p>
      </xsl:when>
      <!-- the fallback option should be a semantically open element such as <ab> -->
      <xsl:otherwise>
        <p facs="#facs_{$numCurr}_{@id}" type="{@type}{$custom?structure?type}">
          <xsl:apply-templates select="p:TextLine" />
        </p>
      </xsl:otherwise>
    </xsl:choose>
