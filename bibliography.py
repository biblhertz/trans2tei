...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', '-i', dest="infile_name", help='Path to mets.xml for input', default="exports/Raphael_in_Early_Modern_Sources,_1483-1602_v__2/bibliography.xml")
    parser.add_argument('--output', '-o', dest="outfile_name", help='Path to output file', default="out/no-outfile-specified-bibliography.xml")
    ...
    xml_data = transform.bibliography_elements(xml_data)
    print(xml_data, file=open("temp/docid.xml", 'w'))

    xml_data = replacements.bibl_label(xml_data)
    print(xml_data, file=open("temp/bibllabel.xml", 'w'))

    xml_data = replacements.bibl_author(xml_data)
    print(xml_data, file=open("temp/biblauthor.xml", 'w'))

    xml_data = replacements.bibl_pub(xml_data)
    print(xml_data, file=open("temp/biblpub.xml", 'w'))

    xml_data = replacements.bibl_article(xml_data)
